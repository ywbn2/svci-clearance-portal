          ...
          <div className="space-y-3">
            <button onClick={async () => {
              if (await showConfirm(`Are you sure you want to ${signingEnabled ? 'DISABLE' : 'ENABLE'} system-wide manual clearance signing?`)) {
                ...
                if (logAction) logAction({ name: 'Superadmin', roleType: 'Superadmin' }, `System Settings`, `Global Clearance Signing ${newVal ? 'Enabled' : 'Disabled'}`);
              }
            }} className={`flex justify-center items-center gap-2 w-full py-3 rounded-xl font-bold shadow-sm transition-all ${signingEnabled ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400 hover:bg-emerald-200' : 'bg-rose-100 text-rose-800 dark:bg-rose-900/30 dark:text-rose-400 hover:bg-rose-200'}`}>
            ...
            <button onClick={async () => {
              if (await showConfirm("Are you sure you want to clear ALL audit logs? This cannot be undone.", "Clear All Logs", true)) {
                // Delete all logs by fetching IDs first to bypass RLS limitations on wildcard deletes
            ...
            <button onClick={async () => {
              ...
              if (await showConfirm(`Are you sure you want to permanently DELETE ${idsToPurge.length} inactive account(s)? This cannot be undone.`)) {
                ...
                if (!hasError) showToast(`Successfully purged ${idsToPurge.length} inactive student accounts.`);
                else showToast("Partial failure purging accounts.", "error");
              }
            }} className="w-full py-2 bg-white dark:bg-slate-800 border-2 border-amber-300 dark:border-amber-900/50 text-amber-700 dark:text-amber-500 font-bold rounded-xl hover:bg-amber-50 dark:hover:bg-amber-900/20 transition">Purge Inactive Accounts</button>
            ...
            <button onClick={async () => {
              ...
              if (await showConfirm("DANGER: Are you sure you want to permanently wipe ALL student records from the database? This affects all clearances, but keeps your offices and settings intact. This is irreversible!")) {
                ...
                // Split into chunks of 100 to prevent URL length limits in Supabase REST querying
                for (let i = 0; i < allIds.length; i += 100) {
                  await supabase.from('students').delete().in('id', allIds.slice(i, i + 100));
                }
                showToast("All student data wiped successfully.");
                if (logAction) logAction({ name: 'Superadmin', roleType: 'Superadmin' }, `System Wipe`, `Permanently deleted all student accounts.`);
              }
            }} className="flex justify-center items-center gap-2 w-full py-2 bg-rose-600/90 text-white font-bold rounded-xl hover:bg-rose-700 shadow-md transition"><AlertTriangleIcon className="w-4 h-4" /> Wipe All Student Data</button>
          </div>
        </Card>
      </div>
